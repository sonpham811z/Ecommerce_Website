  ...
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Filter by status tab (skip for 'all')
        ...
        selectedOrder.items.forEach(async (item) => {
          ...
          if (item.product_id && !productDetails[item.product_id]) {
            try {
              ...
              if (!error && data) {
                console.log('Found product in products table:', data);
                setProductDetails(prev => ({
                  ...prev,
                  [item.product_id]: data
                }));
              } else {
                console.log("Product not found in main products table, trying other tables");
                // Try to find in laptop table as fallback
                ...
                if (!laptopError && laptopData) {
                  console.log('Found product in laptop table:', laptopData);
                  setProductDetails(prev => ({
                    ...prev,
                    [item.product_id]: {
                      ...laptopData,
    ...
    if (orders && orders.length > 0) {
      ...
      orders.forEach(order => {
        if (order.product_info && order.product_info.id && !productDetails[order.product_info.id]) {
          console.log('Found direct product_info:', order.product_info);
          setProductDetails(prev => ({
            ...prev,
  ...
  const cancelOrderMutation = useMutation({
    mutationFn: async (orderId) => {
      try {
        ...
        console.error("Error in cancel mutation:", err);
        throw err;
      }
    },
    onSuccess: (orderId) => {
      // Update local cache
      queryClient.setQueryData(["user-orders"], (oldData) => {
        if (!oldData) return [];
                ...
                >
                  ...
                  {(order.items?.length > 0 || order.product_info) && (
                    <div className="mt-6 relative overflow-hidden">
                      <div className="absolute inset-0 bg-gradient-to-r from-red-50 to-indigo-50 opacity-50 rounded-2xl"></div>
                  ...
                  {selectedOrderId === order.id && (
                    ...
                    >
                      ...
                      <div className="p-6">
                        {/* Order items with improved image display */}
