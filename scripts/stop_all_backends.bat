@echo off
echo ===================================================================
echo Stopping All Backend Services - ChatBot, QR Code, and Invoice
echo ===================================================================
echo.
...
:: Find and kill Invoice API process (running on port 8002)
for /f "tokens=5" %%p in ('netstat -ano ^| findstr :8002 ^| findstr LISTENING') do (
    echo Found Invoice API process with PID: %%p
    taskkill /PID %%p /F
    if %ERRORLEVEL% EQU 0 (
        echo Successfully stopped Invoice API service.
    ) else (
        echo Failed to stop Invoice API service.
    )
)
